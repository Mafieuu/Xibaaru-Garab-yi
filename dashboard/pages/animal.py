...
dash.register_page(
    ...
    path='/animal',
    name='Animal',
    title='Food Footprint - Animal Products'
)
...
layout = html.Div([
    ...
    html.Div([
        html.Div([
            ...
            # Sunburst charts spécifiques aux produits animaux - Utilisation du Store pour récupérer les données
            html.Div([
                # Nous allons utiliser un callback pour filtrer les données par origine
                dcc.Store(id='animal-emissions-data'),
                dcc.Store(id='animal-water-data'),
...
from callbacks.animal_page_callbacks.bar_callback import register_bar_callbacks
from callbacks.animal_page_callbacks.slider_year_callback import register_slider_callbacks
from callbacks.animal_page_callbacks.carte_callback import register_map_callbacks
...
@callback(
    Output('animal-emissions-data', 'data'),
    Input('store-data', 'data')
)
def filter_emissions_data(data_dict):
    # Cette fonction sera implémentée pour filtrer les données de emissions par origine 'Animal'
...
@callback(
    Output('animal-water-data', 'data'),
    Input('store-data', 'data')
)
def filter_water_data(data_dict):
    # Cette fonction sera implémentée pour filtrer les données de water par origine 'Animal'
    # Vous devrez adapter cette logique en fonction de votre structure de données
    return data_dict  # Filtrez ici selon votre structure

@callback(
    Output('emissions-sunburst-container', 'children'),
    Input('animal-emissions-data', 'data')
)
